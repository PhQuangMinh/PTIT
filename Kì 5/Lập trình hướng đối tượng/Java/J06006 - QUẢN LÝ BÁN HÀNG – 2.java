...
class HD{
    String ma, ten, dc, tenmh;
    int ln, sl, tien;
    HD(String ma, String ten, String dc, String tenmh, int sl, int tien, int ln){
        this.ma = ma;
        this.ten = ten;
        this.dc = dc;
        this.tenmh = tenmh;
    ...
    public String toString(){
        return ma + " " + ten + " " + dc + " " + tenmh + " " + sl + " " + tien + " " + ln;
    }
}
public class Main {
    ...
    public static void solve(Scanner scanner) throws ParseException {
        ...
        for (int i=1;i<=k;i++){
            ...
            KH kh = mp.get(makh);
            MH mh = mp2.get(mamh);
            list.add(new HD(maHD, kh.ten, kh.que, mh.ten, sl, sl*mh.ban, (mh.ban-mh.mua)*sl));
        }
        list.sort((o1, o2) -> Integer.compare(o2.ln, o1.ln));
        for (HD hd : list){
